...
export function ImageGeneration({ loggedIn, logout, login }) {
...
function onFileChange(event)  {
  ...
  function handleSubmit(e) {
    ...
    var claimUrl = "https://api.openai.com/v1/images/generations";
  ...
  //console.log(voice);
    ...
    const fetchMethod = {
			method: "POST",
			headers: headers,
      body: JSON.stringify({"prompt":userText,"model":"dall-e-3","n": 1,"size": "1024x1024"}),
  ...
  return (
    ...
    {/* <div style={{
      ...
      <label>
          Image Generation Open API
      </label>
      <br></br><label>
          Please Enter text:
      </label>
      <textarea id="w3review" name="w3review" rows="4" cols="50" value={userText} onChange={handleUserTextChange}>
...
<div>
      {userData.data?.map((item, index) => (
        <div key={index}>
          ...
          <img
            src={item.url}
            alt={`Heroic Figure ${index + 1}`}
            style={{ width: '100%', maxWidth: '600px' }} // You can adjust the styles as needed
          />
          {/* <p>{item.revised_prompt}</p> */}
        </div>
      ))}
