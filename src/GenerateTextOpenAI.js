...
export function GenerateTextOpenAI({ loggedIn, logout, login }) {
...
function onFileChange(event)  {
  ...
  function handleSubmit(e) {
    ...
    var claimUrl = "https://api.openai.com/v1/chat/completions";
...
<div>
      {userData.choices?.map((item, index) => (
        <div key={index}>
          {/* <p>{item.output}</p>
          ...
          <div>{userData.choices[index].message.content}</div>
