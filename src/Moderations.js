...
export function Moderations({ loggedIn, logout, login }) {
...
function onFileChange(event)  {
  ...
  function handleSubmit(e) {
    ...
    var claimUrl = "https://api.openai.com/v1/moderations";
  ...
  return (
    ...
    placeholder="Select a City" className="w-full md:w-14rem" /> */}
     ...
     {/* <input type="file" onChange={onFileChange}/> */}
      ...
      {/* <p>{JSON.stringify(userData.results)}</p>     */}
      ...
      {userData.results?.map((item, index) => (
        <div key={index}>
          ...
          <p>Flagged Harassment? :{item.categories.harassment+ ''}</p>

          <p>Score Harassment : {item.category_scores.harassment}</p>
          <p>Flagged hate? :{item.categories.hate+ ''}</p>
          <p>Score hate : {item.category_scores.hate}</p>
